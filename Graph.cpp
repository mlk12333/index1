...
vector<int> Graph::getAllCore(double parameter1,double parameter2, int corei){
    int l = (int)core_order1[corei].size();
    vector<int> single;
    vector<int> out;
    for(int i = 0;i<l;i++){
        if(core_order1[corei][i][0]<parameter1) break;
        single.push_back(core_order1[corei][i][1]);
    }
    
    for(int i = 0;i<l;i++){
        if(core_order2[corei][i][0]<parameter2) break;
        single.push_back(core_order2[corei][i][1]);
    }
